...
2. **The standalone release binary** for macOS and Linux on arm64 or x86_64. It is fetched as an archive from the matching GitHub release, verified against the published SHA-256 digest, and unpacked with `tar` under `$XDG_CACHE_HOME/betterborg/cli/<version>`, or `~/.cache/betterborg/cli/<version>`, so later runs reuse it. An archive that fails verification is discarded, never unpacked.
