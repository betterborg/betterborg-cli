...
from betterborg_cli.repo_analysis import DIMENSIONS, AnalyzerError, run_analyzer
...
def _commit_repository(repo: Path) -> str:
    ...
    subprocess.run(
        ["git", "-C", str(repo), "config", "user.name", "BetterBorg Tests"],
        check=True,
    )
    subprocess.run(
        [
...
def test_dynamic_selected_agent_persists_append_only_history_and_prompts(
    ...
    adapter = MockAdapter().queue(MockResponse(dynamic=response(first_payload)))
    ...
    with SqliteStore.open(database) as store:
        ...
        assert all(call.model == "selected-model" for call in adapter.calls)
        assert all(
...
def test_invalid_analyzer_output_is_not_persisted(tmp_path: Path) -> None:
    repo_root = tmp_path / "repo"
    repo_root.mkdir()
    ...
    adapter = MockAdapter().queue(MockResponse(payload=invalid))
